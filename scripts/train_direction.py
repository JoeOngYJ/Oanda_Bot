...
from oanda_bot.ml.models.two_stage import DirectionTCNModel, eval_direction, seed_everything, train_direction_epoch
from scripts.two_stage_training_common import HORIZON_BARS, split_masks_with_embargo


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train direction model (two-stage pipeline).")
...
def _to_dir_class(y_dir: np.ndarray) -> np.ndarray:
    # input convention: -1 invalid, 1 long, 0 short
    out = np.full_like(y_dir, -1, dtype=np.int64)
    out[y_dir == 1] = 0  # long class index
    out[y_dir == 0] = 1  # short class index
    return out


def _make_loader(seq: np.ndarray, ctx: np.ndarray, y_dir: np.ndarray, batch_size: int, shuffle: bool) -> DataLoader:
    ds = TensorDataset(
        torch.as_tensor(seq, dtype=torch.float32),
        torch.as_tensor(ctx, dtype=torch.float32),
        torch.as_tensor(y_dir, dtype=torch.int64),
    )
...
def main() -> None:
    ...
    y_dir = _to_dir_class(z["y_direction"].astype(np.int64))
    ts = z["timestamps"]
    ...
    tr_loader = _make_loader(seq[tr], ctx[tr], y_dir[tr], args.batch_size, shuffle=True)
    va_loader = _make_loader(seq[va], ctx[va], y_dir[va], args.batch_size, shuffle=False)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = DirectionTCNModel(seq_features=seq.shape[2], ctx_dim=ctx.shape[1]).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        tr_losses = [train_direction_epoch(model, b, opt, device, grad_clip=1.0) for b in tr_loader]
        va_losses = [eval_direction(model, b, device) for b in va_loader]
    ...
    model_path = out_dir / f"direction_{stamp}.pt"
    cfg_path = out_dir / f"direction_{stamp}.json"
